...
def find_best_split(feature_vector, target_vector):
    ...
    * :math:`p_1` и :math:`p_0` — доля объектов класса 1 и 0 соответственно.
    ...
    """
    values, counts = np.unique(feature_vector, return_counts=True)
    thresholds = np.delete(values / 2 + np.append((values / 2)[1:], 0), -1)

    left = counts.cumsum()[:-1]
    right = feature_vector.shape[0] - left

    values, counts = np.unique(np.sort(feature_vector), return_index=True)
    target = target_vector[np.argsort(feature_vector)]

    p1_l = target.cumsum()[counts[1:] - 1] / left
    p0_l = 1 - p1_l
    h_l = 1 - (p1_l ** 2) - (p0_l ** 2)

    p1_r = (target_vector.sum() - target.cumsum()[counts[1:] - 1]) / right
    p0_r = 1 - p1_r
    h_r = 1 - (p1_r ** 2) - (p0_r ** 2)

    ginis = -(left / feature_vector.shape[0]) * h_l - (right / feature_vector.shape[0]) * h_r
