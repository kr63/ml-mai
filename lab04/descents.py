...
class BaseDescent:
    ...
    def calc_loss(self, x: np.ndarray, y: np.ndarray) -> floating[Any]:
        ...
        """
        return np.mean((y - x.dot(self.w)) ** 2)
...
class StochasticDescent(VanillaGradientDescent):
    ...
    def calc_gradient(self, x: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        """
        index = np.random.randint(x.shape[0], size=self.batch_size)
        return (1 / self.batch_size) * (2 * x[index].T @ (x[index] @ self.w - y[index]))
...
class Adam(VanillaGradientDescent):
    ...
    def update_weights(self, gradient: np.ndarray) -> np.ndarray:
        ...
        result = m_hat * eta / (v_hat ** 0.5 + self.eps)
        self.w -= result
        return -result
